...
class export_array:
	def __init__(self):
		...
		for i in range(self.maxd):    #day number
			...
			#set excel value
			for k in range(int(self.userinfo.count)):    #round number
	...
	def enumarray_sub(self,arrow,x,y,addedx,addedy,takeout,day):
		...
		if takeout<int(self.userinfo.count) and x<int(self.userinfo.team_count):	#if take out is not enough.
			print "in",x,y,addedx,addedy
			if addedy<2 and y<int(self.userinfo.team_count):	#if this row can be choice, and y is not over array length.
	...
	def enumarray_main(self):
		...
		while True:
			...
			if sum>=int(self.userinfo.count):
